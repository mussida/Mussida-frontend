...
export default function ProfileHome() {
  ...
  const { mutate, isLoading, isError } = useMutation({
    mutationFn: (params: { genre: string }) => {
      ...
      if (selectedGenre) {
        const replacedGenre = copiedFavouriteGenres.indexOf(selectedGenre);
  ...
  return (
    <View style={{ height: "100%" }}>
      ...
      >
        ...
        {[...new Array(3 - (userGenres?.data.length ?? 0))].map((_, index) => {
          return (
            <Button
              onPress={() => {
                bottomSheetRef.current?.expand();
        ...
        <BottomSheetScrollView>
          {genres.map((genre, index) => {
            return (
              <TouchableRipple
                style={{
                  opacity: userGenres?.data.includes(genre) ? 0.5 : 1,
                }}
                onPress={() => {
                  mutate({ genre });
                }}
